...
pub fn new_and_next_test() {
  ...
  iterator.next(yielded, value: None) |> should.equal(iteration.Yield(42))

  let returned = iterator.new(fn(_next) { iteration.Return("done") })
  iterator.next(returned, value: None) |> should.equal(iteration.Return("done"))
}

pub fn stateful_iterator_test() {
  // Pass values through `next` to drive iterator state.
  let iter =
  ...
  let result = iterator.next(iter, value: None)
  should.equal(result, iteration.Yield(0))

  let result = iterator.next(iter, value: Some(0))
  should.equal(result, iteration.Yield(1))

  let result = iterator.next(iter, value: Some(1))
  should.equal(result, iteration.Yield(2))

  let result = iterator.next(iter, value: Some(2))
  should.equal(result, iteration.Yield(3))

  let result = iterator.next(iter, value: Some(3))
  should.equal(result, iteration.Return(Nil))
}

pub fn return_test() {
  let iter = iterator.new(fn(_) { iteration.Yield(1) })
  iterator.return(iter, value: None)
...
pub fn return_with_handler_test() {
  let iter =
    ...
    |> iterator.set_return(fn(_value) { iteration.Return(Nil) })
...
pub fn return_with_value_test() {
  ...
  iterator.return(iter, value: Some(42))
...
pub fn throw_with_handler_test() {
  let iter =
    iterator.new(fn(_) { iteration.Yield(1) })
    |> iterator.set_throw(fn(_err) { iteration.Return(Nil) })
...
pub fn throw_passes_reason_test() {
  let iter =
    iterator.new(fn(_) { iteration.Yield(1) })
    |> iterator.set_throw(fn(err) { iteration.Return(err) })
...
pub fn return_callback_throws_test() {
  let iter =
    iterator.new(fn(_) { iteration.Yield(1) })
    |> iterator.set_return(fn(_value) { panic as "handler boom" })

  let assert Error(_) = iterator.return(iter, value: None)
}
...
pub fn for_test() {
  ...
  iterator.next(iter, value: None) |> should.equal(iteration.Return(Nil))
}

pub fn from_yielder_test() {
  let iter = yielder.from_list([10, 20, 30]) |> iterator.from_yielder

  iterator.next(iter, value: None) |> should.equal(iteration.Yield(10))
  iterator.next(iter, value: None) |> should.equal(iteration.Yield(20))
  iterator.next(iter, value: None) |> should.equal(iteration.Yield(30))
  iterator.next(iter, value: None) |> should.equal(iteration.Return(Nil))
}
