...
pub fn from_list_test() {
  ...
  uint8_array.at(array, 0) |> should.equal(Some(1))
  uint8_array.at(array, 2) |> should.equal(Some(3))
}
...
pub fn at_test() {
  ...
  uint8_array.at(array, 0) |> should.equal(Some(10))
  uint8_array.at(array, -1) |> should.equal(Some(30))
  uint8_array.at(array, 5) |> should.equal(None)
}
...
pub fn slice_test() {
  ...
  uint8_array.at(copy, 0) |> should.equal(Some(1))
}
...
pub fn slice_range_test() {
  ...
  uint8_array.at(sliced, 0) |> should.equal(Some(2))
  uint8_array.at(sliced, 1) |> should.equal(Some(3))
}

pub fn fill_test() {
  let array = uint8_array.from_length(3)
  uint8_array.fill(array, 42)
  uint8_array.at(array, 0) |> should.equal(Some(42))
  uint8_array.at(array, 2) |> should.equal(Some(42))
}

pub fn reverse_test() {
  let array = uint8_array.from_list([1, 2, 3])
  uint8_array.reverse(array)
  uint8_array.at(array, 0) |> should.equal(Some(3))
  uint8_array.at(array, 2) |> should.equal(Some(1))
}

pub fn with_test() {
  let array = uint8_array.from_list([1, 2, 3])
  let updated = uint8_array.with(array, 1, 42)
  uint8_array.at(updated, 1) |> should.equal(Some(42))
  uint8_array.at(array, 1) |> should.equal(Some(2))
}
...
pub fn set_test() {
  ...
  uint8_array.at(array, 0) |> should.equal(Some(10))
  uint8_array.at(array, 1) |> should.equal(Some(20))
  uint8_array.at(array, 2) |> should.equal(Some(0))
}

pub fn copy_within_test() {
  let array = uint8_array.from_list([1, 2, 3, 4, 5])
  uint8_array.copy_within(array, 0, 3)
  uint8_array.at(array, 0) |> should.equal(Some(4))
  uint8_array.at(array, 1) |> should.equal(Some(5))
}
...
pub fn find_test() {
  let array = uint8_array.from_list([1, 2, 3])
  uint8_array.find(array, fn(value) { value > 1 }) |> should.equal(Some(2))
  uint8_array.find(array, fn(value) { value > 5 }) |> should.equal(None)
}
...
pub fn filter_test() {
  ...
  uint8_array.at(filtered, 0) |> should.equal(Some(4))
}

pub fn map_test() {
  let array = uint8_array.from_list([1, 2, 3])
  let mapped = uint8_array.map(array, fn(value) { value * 2 })
  uint8_array.at(mapped, 0) |> should.equal(Some(2))
  uint8_array.at(mapped, 2) |> should.equal(Some(6))
}

pub fn index_map_test() {
  let array = uint8_array.from_list([10, 20, 30])
  let mapped = uint8_array.index_map(array, fn(value, index) { value + index })
  uint8_array.at(mapped, 0) |> should.equal(Some(10))
  uint8_array.at(mapped, 1) |> should.equal(Some(21))
  uint8_array.at(mapped, 2) |> should.equal(Some(32))
}
...
pub fn to_sorted_test() {
  ...
  uint8_array.at(array, 0) |> should.equal(Some(3))
  uint8_array.at(sorted, 0) |> should.equal(Some(1))
}
...
pub fn to_reversed_test() {
  ...
  uint8_array.at(array, 0) |> should.equal(Some(1))
  uint8_array.at(reversed, 0) |> should.equal(Some(3))
}
