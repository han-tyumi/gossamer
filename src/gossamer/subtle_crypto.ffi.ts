...
export const digest: typeof $subtleCrypto.digest = (algorithm, data) => {
  return subtle.digest(algorithm, data as unknown as BufferSource);
};
...
export const export_key_jwk: typeof $subtleCrypto.export_key_jwk = (key) => {
  return subtle.exportKey("jwk", key) as Promise<unknown>;
};
...
export const derive_bits: typeof $subtleCrypto.derive_bits = (
  ...
  return subtle.deriveBits(toDeriveAlgorithm(algorithm), baseKey, length);
};
...
export const wrap_key_jwk: typeof $subtleCrypto.wrap_key_jwk = (
  ...
  return subtle.wrapKey("jwk", key, wrappingKey, toWrapAlgorithm(algorithm));
};
