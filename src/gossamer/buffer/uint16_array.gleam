...
/// Creates a `Uint16Array` view over `buffer`. Returns `Detached` if
...
pub fn from_buffer(buffer: ArrayBuffer) -> Result(Uint16Array, BufferError)

/// Creates a `Uint16Array` view over a slice of `buffer` starting at
...
/// `buffer`.
///
@external(javascript, "./uint16_array.ffi.mjs", "from_buffer_range")
pub fn from_buffer_range(
  buffer: ArrayBuffer,
  byte_offset byte_offset: Int,
  length length: Int,
) -> Result(Uint16Array, BufferError)
...
pub fn bytes(array: Uint16Array) -> Result(Uint8Array, BufferError)
